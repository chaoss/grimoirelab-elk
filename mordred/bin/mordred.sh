...
function compose_gmane_list {
    ...
    for project in $(cat $1);
    do
      ./projectinfo.py $input_file list --project_name $project --repo gmane >> $2
    done
}

function compose_pipermail_list {
    input_file=`echo $PROJECTS_JSON_FILE | cut -d / -f 3-`
    cd ~/GrimoireELK/mordred/utils/
    ./projectinfo.py $input_file list > $1
    IFS=$'\n'
    for project in $(cat $1);
    do
      ./projectinfo.py $input_file list --project_name $project --repo pipermail >> $2
    done
}
...
function gmane_retrieval {
    ...
    compose_gmane_list $TMP_GMANE_PROJECT_LIST $TMP_GMANE_LIST
    cd ~/GrimoireELK/utils/
    for url in $(cat $TMP_GMANE_LIST);
    do
        ./p2o.py -e $ES_URI -g --index $GMANE_INDEX gmane $url $FROM_DATE_STRING $FROM_DATE >> $LOGS_DIR"/mls-collection.log" 2>&1
    done
...
function pipermail_retrieval {
    ...
    compose_pipermail_list $TMP_PIPERMAIL_PROJECT_LIST $TMP_PIPERMAIL_LIST
    cd ~/GrimoireELK/utils/
