...
class TestGit(TestBaseBackend):
    ...
    def test_enrich_areas_of_code(self):
        ...
        source = response['git_aoc-enriched']['settings']['index']
        self.assertIn('creation_date', source)
        self.assertIn('number_of_shards', source)
        self.assertIn('number_of_replicas', source)
        self.assertIn('provided_name', source)
